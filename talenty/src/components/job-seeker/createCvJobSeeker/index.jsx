...
export default function CreateCvJobSeeker({ isApplyingId, isMatched }) {
  ...
  const tempTemplateData = useSelector(
    (state) => state.globalData.templateData
  );
  const templateData = useMemo(() => {
    ...
    return {
      ...tempTemplateData,
      fields: tempTemplateData.fields.filter((el) => el).filter(filterUserCv),
    };
  }, [tempTemplateData]);
  ...
  }, [isApplyingId, isMatched, dispatch]);
  // update local storage whenever data changed and also redux
  useEffect(() => {
    if (!isApplyingId) {
      if (templateData === null) {
        dispatch(
          userInfo.cvTemplateId
            ? getEditedUserCv(userInfo.cvTemplateId)
            : getTemplateActions()
        );
      }
    }
  ...
  return (
    <Box sx={{ pt: '44px', pl: '52px', pr: '52px', minHeight: '100vh' }}>
      <Pagination
        pagesCount={templateData?.fields.length || 0}
        {...{ exactPage }}
      />
      {/* body */}
      <Box
      ...
      >
        ...
        >
          <SharedTemplateHeader
            title={templateData?.fields[exactPage - 1]?.name}
          />
        ...
        <Box sx={{ display: 'flex', pt: '44px', pb: '40px' }}>
          ...
          >
            <AddButton />
            {templateData?.fields &&
            templateData.fields.length !== exactPage ? (
              <Button
                sx={HOME_PRIMARY_BUTTON}
                onClick={() => {
